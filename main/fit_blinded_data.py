...
if __name__ == '__main__':
    ...
    parser.add_argument('--regions', nargs='+', type=str, default=['NGC', 'SGC'], choices=['NGC', 'SGC', 'GCcomb'] , help='Sky regions to include.')
    ...
    if args.zrange is None:
        for tr in args.tracers:
            for zmin, zmax in default_bins[tr]:
                tracers_bins.append((tr, (zmin, zmax)))
    else:
        if len(args.zrange) != len(args.tracers):
            raise ValueError("Number of --zrange entries must match number of --tracers")
        for tr, zr in zip(args.tracers, args.zrange):
            zmin, zmax = map(float, zr.split(','))
            tracers_bins.append((tr, (zmin, zmax)))
    ...
    for (tracer, zrange), region, weight_type, approach in itertools.product(tracers_bins, args.regions, args.weight_types, args.approaches):
        ...
        chain_fn    = RESULT_DIR+f'/full-shape/mcmc/chain_{task}_{tracer}_z{zrange[0]:.1f}-{zrange[1]:.1f}_{region}_{weight_type}{auw}{cut}.npy'
        if approach in ['FM', 'SF']:
            corr_type = 'pk'
            bins_type = 'y3_blinding'
            (kmin, kmax, kbin, lenk) = load_bins(corr_type, bins_type)
            if use_emulator:
                emulator_fn = RESULT_DIR+f'/emulator/emulator_{task}_{tracer}_z{zrange[0]:.1f}-{zrange[1]:.1f}_{region}_{weight_type}{auw}{cut}_k{kmin}-{kmax}.npy'
